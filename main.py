...
if is_android():
    ...
    def android_show_print_review_and_print(self, oid, cust, box_n):
        ...
        def _print_sequence(mac):
            success_count=0; fail_count=0; err_msgs=[]
            for i in range(box_n):
                payload = escpos_bytes_for_label(oid,cust,i+1,box_n)
                ok,err = print_via_bluetooth_pyjnius(mac,payload)
                time.sleep(0.15)
                if ok: success_count+=1
                else: fail_count+=1; err_msgs.append(f"#{i+1}: {err}")
            if success_count>0: add_history_entry(oid,cust,box_n)
            res=f"In xong: {success_count} thành công, {fail_count} lỗi"
            if err_msgs: res+="\n"+("\n".join(err_msgs))
            status.text=res
        def on_print(*_):
            devices = find_paired_printers_pyjnius()
            if not devices: status.text="Chưa ghép đôi máy in"
            else: _print_sequence(devices[0][1])
        btn_print.bind(on_release=on_print)
